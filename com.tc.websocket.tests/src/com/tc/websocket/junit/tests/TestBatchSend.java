...
public class TestBatchSend {
	...
	@Test
	public void testBatchSend() throws InterruptedException, JsonParseException, JsonMappingException, IOException {
		
	
		//make sure the listners, observers, intervaled are loaded so we get a response.
		...
		List<SocketMessage> batch = new ArrayList<SocketMessage>();
		
		//built the batch of messages.
		for(int i =0;i< 10; i++){
		...
		client.send(JSONUtils.toJson(batch));
