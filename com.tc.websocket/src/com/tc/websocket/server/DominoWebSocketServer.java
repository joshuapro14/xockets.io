...
public class DominoWebSocketServer implements IDominoWebSocketServer, Runnable {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#
	 * getWebSocketAndObserverCount()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#removeUser(java.lang.
	 * String)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#removeUser(com.tc.
	 * websocket.valueobjects.IUser)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#getUsersOnThisServer()
	 */
	@Override
	@Stopwatch(time = 10)
	...
	public void onOpen(ContextWrapper conn, FullHttpRequest req) {
		...
		// now that we have the user fully initialized, make sure they have at
		// least reader access
		...
		// check to see if there's any messages waiting on this event for this
		// user.
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#closeWithDelay(com.tc.
	 * websocket.server.ContextWrapper, int)
	 */
	@Override
	public void closeWithDelay(final ContextWrapper conn, int delay) {
		...
		if (user != null && ServerInfo.getInstance().isCurrentServer(user.getHost())) {
			user.setGoingOffline(true);
			TaskRunner.getInstance().add(new ApplyStatus(user), delay);// mark
																		// user
																		// as
																		// offline
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#onMessage(java.lang.
	...
	private void processMessage(IUser user, SocketMessage msg, SocketMessage message) {
		...
		// check to see if there's any messages waiting on this event for
		// receiver
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#resolveUser(com.tc.
	 * websocket.server.ContextWrapper)
	 */
	@Override
	@Stopwatch
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#resolveUser(java.lang.
	 * String)
	 */
	@Override
	@Stopwatch
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#containsUser(java.lang.
	 * String)
	 */
	@Override
	@Stopwatch
		...
		try {
			...
			for (IUser user : col) {
				...
				if (path.isWild() && user.startsWith(path.getUri())) {
					if (path.hasRoles()) {
						// role filters were supplied and the user is part of
						// one of the roles
	...
	private boolean sendToUri(String uri, SocketMessage msg) {
			...
			for (IUser user : list) {
				...
				if (!user.getUserId().startsWith(StringCache.FORWARD_SLASH)) {
					if (this.send(user.getUserId(), msg)) {
						b = true; // if the message was sent to anyone in the
									// uri we're consider it sent.
					}
				}
			}
		}
		...
		try {
			...
			for (Script script : scripts) {
				// SocketMessage msg = JSONUtils.toObject(json,
				// SocketMessage.class);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#queueMessage(com.tc.
	 * websocket.valueobjects.SocketMessage)
	 */
	@Override
	@Stopwatch
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#isValidSize(java.lang.
	 * String)
	 */
	@Override
	@Stopwatch
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#removeEventObserver(com.tc
	 * .websocket.scripts.Script)
	 */
	 ...
	 * 
	 * @see
	 * com.tc.websocket.server.IDominoWebSocketServer#addUriListener(com.tc.
	 * websocket.scripts.Script)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tc.websocket.server.IDominoWebSocketServer#addIntervaled(com.tc.
	 * websocket.scripts.Script)
	 */
	@Override
